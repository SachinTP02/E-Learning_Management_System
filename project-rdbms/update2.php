<?php
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['p']== 5 && $_SESSION['key']=='admin') 
    {
      $eid=@$_GET['nid'];
      
      $r = mysqli_query($con,"DELETE FROM note WHERE nid='$eid' ") or die('Error');
      header("location:addnotes.php?q=5");
    }
  }

  if(isset($_SESSION['key']))
  {
    if(@$_GET['q']== 5 && $_SESSION['key']=='admin') 
    {
      $eid=@$_GET['nid'];
      
      $r = mysqli_query($con,"DELETE FROM notes WHERE nid='$eid' ") or die('Error');
      header("location:addnotes.php?q=5");
    }
  }
  ...
  if(isset($_SESSION['key']))
  {
    if(@$_GET['p']== 4 && $_SESSION['key']=='admin') 
    {
      $name = $_POST['name'];
      ...
      $id=uniqid();
      $q3=mysqli_query($con,"INSERT INTO note VALUES  ('','$stream','$sem','$subject','$module','$name' ,'$file', '$uploadedby' , '$extra', NOW())");
